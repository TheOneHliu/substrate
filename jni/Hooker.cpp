        ...
        for (unsigned offset(0); offset != used; ++offset) {
            if (T$pcrel$ldr(backup[offset])) {
                ...
                *--trailer = ((reinterpret_cast<uint32_t>(area + offset) + 4) & ~0x2) + (bits.u == 0 ? -exts.immediate : exts.immediate);
                ...
                if (bits.h1) {
                    LOGI("MS:Error:pcrel(%u):add (rd > r7)\n", offset);
                    goto fail;
                }
...
#if 0
        LOGI("1212 result = %p %p", *result, buffer);
        int i;
        for (i = 0; i < length; i++)
        		LOGI("1212buffer(%08x)0x%02x ", buffer + i, buffer[i]);
#endif
    }
}
...
extern void MSHookFunction(void *symbol, void *replace, void **result) {
    ...
    while (used < required) {
        ...
        if (width == 0) {
            LOGI("MS:Error:MSGetInstructionWidthIntel(%p) == 0", area + used);
            return;
        }
    ...
    for (size_t offset(0), width; offset != used; offset += width) {
        ...
        if ((decode.modrm & 0xc7) == 0x05) {
            if (decode.opcode == 0x8b) {
                ...
                LOGI("MS:Error: Unknown RIP-Relative (%.2x %.2x)\n", decode.opcode, decode.opcode2);
                continue;
            }
        } else
#endif
